...
class DocumentCollaboratorsWidget extends Widget {
  ...
  /** Shared model instance from the document context */
  ...
  private _updateCollaboratorsFromAwareness(): void {
    ...
    awarenessStates.forEach((state: any, clientId: number) => {
      ...
      let color =
        user.color || state.color || user.cursorColor || state.cursorColor;
  ...
  private _renderCollaborators(): void {
    ...
    const visibleCollaborators = collaborators.slice(
      0,
      this._maxVisibleCollaborators
    );
    const remainingCount = Math.max(
      0,
      collaborators.length - this._maxVisibleCollaborators
    );
    ...
    if (remainingCount > 0) {
      const moreIcon = this._createMoreIcon(
        remainingCount,
  ...
  private _showMoreModal(targetElement: HTMLElement): void {
    ...
    const hiddenCollaborators = Array.from(this._collaborators.values()).slice(
      this._maxVisibleCollaborators
    );

    const modal = document.createElement('div');
    modal.className =
      'jp-DocumentCollaborators-modal jp-DocumentCollaborators-modal-more';
